...
from .request import AccessCode, ErrorMessage, ErstevakRequest
...
class ErstevakVacuumGauge(RS485Client):
    """
    Erstevak vacuum gauge RS485 V2 client.
    """
            ...
            try:
                ...
                if response and hasattr(response, "data"):
                    ...
                    if response.function_code == 7:
                        self.logger.error(
                            "ERROR: %s", ErrorMessage.from_str(response.data).description()
                        )
        ...
        if response:
            if check_checksum(response[:-2], response[-2]):
                ...
                if result["access_code"] == 7:  # Error
                    self.logger.error(
                        "ERROR: %s", ErrorMessage.from_str(result["data"]).description()
                    )
            else:
