"""
Erstevak RS485 Version 1 Client Module.

This module provides a custom RS485 client for interacting with an Erstevak vacuum gauge (e.g.,
...
`erstevak.rs485.v1` subpackage to manage gauge operations such as pressure measurement,
calibration, setpoint adjustments, and Penning gauge control.

Classes:
    ErstevakVacuumGauge: An RS485 client class for Erstevak vacuum gauges, providing methods
...
class ErstevakVacuumGauge(RS485Client):
    """
    Erstevak vacuum gauge RS485 client.

    A custom RS485 client for interacting with an Erstevak vacuum gauge over its RS485 protocol.
    It extends `scietex.hal.serial.client.RS485Client` with Erstevak-specific framing
    (`ErstevakASCIIFramer`), decoding (`ErstevakDecodePDU`), and request handling
    (`ErstevakRequest`). The client supports two backends: `pymodbus` for Modbus-style
    ...
    label : str
        A label for the gauge (default "Erstevak Gauge"), inherited from `RS485Client`.
    ...
    def __init__(
        ...
        """
        Initialize an ErstevakVacuumGauge client instance.
        ...
        Erstevak-specific framer, decoder, and response type.
        ...
        label : Optional[str], optional
            A custom label for the gauge. Defaults to "Erstevak Gauge" if None.
        ...
        super().__init__(
            ...
            address=address,
            label=_label,
            custom_framer=ErstevakASCIIFramer,
            custom_decoder=ErstevakDecodePDU,
            custom_response=[ErstevakRequest],
            logger=logger,
        )
    ...
    async def request_gauge(self, command: str, data: Optional[bytes] = None) -> Optional[str]:
        ...
        """
        result = None
        request = ErstevakRequest(command=command, data=data, slave=self.address, transaction=0)
